 ...
 */
package com.github.jspeccyremastered.machine;

import com.github.jspeccyremastered.gui.JSpeccySettings;
import com.github.jspeccyremastered.gui.JSpeccyScreen;
...
import com.github.jspeccyremastered.joystickinput.JoystickRaw;
import com.github.jspeccyremastered.machine.Keyboard.JoystickModel;
import com.github.jspeccyremastered.snapshots.SpectrumState;
...
import com.github.jspeccyremastered.z80core.Z80;

/**
 *
 * @author jsanchez
 */
public class Spectrum extends com.github.jspeccyremastered.z80core.MemIoOps implements Runnable, com.github.jspeccyremastered.z80core.NotifyOps {
    ...
    private final JSpeccySettings specSettings;
    /* Config vars */
...
//            Logger.getLogger(Spectrum.class.getName()).log(Level.SEVERE, null, ex);
        ...
        keyboard = new Keyboard(settings, joystick1, joystick2);
    ...
    public final SpectrumState getSpectrumState() {
        ...
        if (connectedIF1) {
            state.setNumMicrodrives(settings.getMicrodriveUnits());
        }
    ...
    public final void setSpectrumState(SpectrumState state) {
        ...
        settings.setIssue2(state.isIssue2());

        enabledAY = state.isEnabledAY();
        if (enabledAY) {
            ay8912.setSpectrumModel(spectrumModel);
            ay8912.setAY8912State(state.getAY8912State());
            settings.setAyEnabled48K(state.isEnabledAYon48k());
        }

        settings.setConnectedIF1(state.isConnectedIF1());
        if (state.isConnectedIF1()) {
            settings.setMicrodriveUnits(state.getNumMicrodrives());
        }
    ...
    public void selectHardwareModel(MachineTypes hardwareModel) {
        ...
        switch (spectrumModel.codeModel) {
            case SPECTRUM48K:
                ...
                connectedIF1 = settings.isConnectedIF1();
                Paleta = Paleta48k;
                break;
            case SPECTRUM128K:
    ...
    public final void loadConfigVars() {
        ...
        keyboard.setMapPCKeys(settings.isMapPCKeys());

        z80.setBreakpoint(0x0066, specSettings.isMultifaceEnabled());
        
        saveTrap = settings.isEnableSaveTraps();
        z80.setBreakpoint(0x04D0, saveTrap);
        
        loadTrap = settings.isEnableLoadTraps();
        z80.setBreakpoint(0x0556, loadTrap);
        
        flashload = settings.isFlashLoad();
        
        if1.setNumDrives(settings.getMicrodriveUnits());
        if (spectrumModel.codeModel != MachineTypes.CodeModel.SPECTRUMPLUS3) {
            connectedIF1 = settings.isConnectedIF1();
        } else {
            connectedIF1 = false;
...
//    }
    ...
    private void enableSound() {
        ...
        audio.open(spectrumModel, ay8912, enabledAY,
                settings.isHifiSound() ? 48000 : 32000);
...
//                spectrumModel.firstBorderUpdate, spectrumModel.lastBorderUpdate));
    ...
    private void unpageLec() {
        ...
        enabledAY = settings.isAyEnabled48K();
    }
